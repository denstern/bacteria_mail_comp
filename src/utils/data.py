...
class Data(data.Dataset):
    def __init__(self, pathes_shapes, img_size=(256, 512)):
    ...
    def __getitem__(self, index):
        img = cv2.imread(self.pathes[index][0]).astype(np.float32) / 255 - 0.5
        img = torch.from_numpy(img.transpose(2, 0, 1))
        msk = (cv2.imread(self.pathes[index][1]) / 255).astype(np.int)
        msk = torch.from_numpy(msk.transpose(2, 0, 1))[0,:,:].unsqueeze(0)
        logger.info('Image shape {}, mask shape {}'.format(img.shape, msk.shape))
